import pytest
from inventory.models import Reagent, Category, Supplier, StockLot, Location, User, Requisition, AuditLog, StockMovement
...
def test_fefo_withdrawal_logic():
    ...
    selected_lots_info_1 = get_lots_for_withdrawal(reagent, Decimal('50.00'))
    ...
    selected_lots_info_2 = get_lots_for_withdrawal(reagent, Decimal('150.00'))
    ...
    with pytest.raises(ValueError, match="Not enough stock"): # Expect ValueError
        get_lots_for_withdrawal(reagent, Decimal('500.00')) # Total available is 300
...
def test_get_lots_for_withdrawal_no_stock():
    ...
    reagent = Reagent.objects.create(name='Hidróxido de Amônio', sku='NH4OH-001', category=category, supplier=supplier, min_stock_level=100)
    
    with pytest.raises(ValueError, match="Not enough stock"):
        get_lots_for_withdrawal(reagent, Decimal('1.00'))
...
def test_approve_requisition_invalid_status():
    ...
    reagent = Reagent.objects.create(name='Reagente Aprovado', sku='RA-001', category=Category.objects.create(name='Cat A'), supplier=Supplier.objects.create(name='Sup A'), min_stock_level=1)
    requisition = Requisition.objects.create(requester=requester, reagent=reagent, quantity=1, status='Aprovada')
...
def test_perform_withdrawal_updates_stock():
    """
    Tests that perform_withdrawal correctly updates stock quantities and creates StockMovement records.
    """
    # Create prerequisites
    ...
    perform_withdrawal(reagent, withdrawal_qty, requester)
    ...
    movements = StockMovement.objects.filter(user=requester, move_type='Retirada').order_by('timestamp')
    assert movements.count() == 2
    assert movements[0].stock_lot == lot1
    assert movements[0].quantity == Decimal('100.00')
    assert movements[1].stock_lot == lot2
    assert movements[1].quantity == Decimal('20.00')
